...
> {-# LANGUAGE TypeFamilies, FlexibleContexts, RebindableSyntax, QuasiQuotes #-}
...
>                        return $ a + b + c
 ...
 fooFwdC :: Num a => StencilC a (Forward (S (S Z))) x a

> fooFwdC = StencilC $ [codo| x => a <- ixC (Pos Z) x
>                                  b <- ixC (Pos (S Z)) x
>                                  c <- ixC (Pos (S (S Z))) x
>                                  (counit a) + (counit b) + (counit c) |]
...
>     type CPlus c s t
>     type CUnit c
>     coextend :: (c s a -> b) -> c (CPlus c r s) a -> c r b
>     counit :: c (CUnit c) a -> a

>     cmap :: (a -> b) -> c r a -> c r b
>     
...
> data ArrayCoReader e r e' = CoR (SpecArray r e, e')

> instance IxComonad (ArrayCoReader x) where
>     type CPlus (ArrayCoReader x) s t = Append s t
>     type CUnit (ArrayCoReader x)     = HNil

>     coextend k (CoR (MkSA a, x)) = CoR (MkSA a, k (CoR (MkSA a, x)))
>     counit (CoR (_, x)) = x
>     cmap f (CoR (a, x)) = (CoR (a, f x))
...
> data StencilC a r x y where
>     StencilC :: (ArrayCoReader a spec x -> y) -> StencilC a (Sort spec) x y
...
> ixC :: IntT x -> ArrayCoReader a (HCons x t) b -> a
> ixC n (CoR (MkSA (a, cursor), _)) = a ! (cursor + toValue n)
